<x-layouts.general-client :title="'Help Center'">
    <div class="flex flex-row w-full gap-6 p-4 md:p-6 h-fit" x-data="{
    activeTab: 'all',
    openFaq: null,
    userInput: '',
    messages: [],
    ...
    sendMessage() {
        ...
        this.messages.push({
            ...
            text: matchingFaqs.length
                ...
                : 'I couldn’t find specific information. Try rephrasing or call support at +2355555888.',
    ...
    get chatPairsCount() {
        ...
        <div class="w-full max-w-4xl mx-auto">
            {{-- Header --}}
            <div class="mb-8">
                <div class="text-sm text-gray-600 dark:text-gray-400 mb-2">Firecrew Customer Service</div>
                <h1 class="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">Hi, How can we help you?
                </h1>
                <p class="text-gray-600 dark:text-gray-400 mb-6">We are available 24/7 to answer your inquiries.</p>

                <div class="mb-8 max-w-3xl">
                    {{-- Clear Messages Button --}}
                    <button @click="clearMessages()" x-show="messages.length > 0"
                        class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-red-500 mb-6 transition-colors">
                    ...
                    {{-- Chat Container --}}
                    <div id="chat-container" class="space-y-4 mb-6 transition-all duration-300" :class="{
                             'overflow-visible': chatPairsCount <= 1,
                             'max-h-[300px] w-full overflow-y-auto': chatPairsCount > 1
                         }" x-show="messages.length > 0">
                        <template x-for="(message, index) in messages" :key="index">
                            <div>
                                <div x-show="message.type === 'user'" class="flex justify-end mb-4">
                                    <div
                                        class="bg-indigo-600 text-white rounded-2xl rounded-tr-sm px-4 py-3 max-w-[80%] shadow-sm">
                                ...
                                <div x-show="message.type === 'bot'" class="flex gap-3 mb-4">
                                    <div
                                        class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center flex-shrink-0">
                                    ...
                                    <div
                                        class="bg-gray-100 dark:bg-gray-800 rounded-2xl rounded-tl-sm px-4 py-3 max-w-[80%]">
                                        ...
                                        <div x-show="message.faqs && message.faqs.length > 0" class="mt-3 space-y-2">
                                            <template x-for="faq in message.faqs" :key="faq.id">
                                                <button @click="expandFaq(faq)"
                                                    class="block w-full text-left text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
                    ...
                    <div x-show="expandedFaq" x-transition
                        class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-100 dark:border-indigo-800 p-4 mb-6">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="font-semibold text-indigo-900 dark:text-indigo-200 text-sm"
                                x-text="expandedFaq?.question"></h3>
                            <button @click="expandedFaq = null" class="text-indigo-400 hover:text-indigo-600"><svg
                    ...
                    <div class="mb-4" x-show="messages.length === 0">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <template x-for="faq in faqs.slice(4,7)" :key="'suggest-'+faq.id">
                                <button @click="selectPrompt(faq.question)"
                                    class="px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-full text-xs text-gray-600 dark:text-gray-300 hover:border-indigo-500 hover:text-indigo-600 transition-all shadow-sm">
                                    <span x-text="faq.question"></span>
                                </button>
                            </template>
                        </div>
                    ...
                    <div class="relative">
                        <div
                            class="flex items-center gap-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 p-2 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
                            <input type="text" x-model="userInput" @keydown.enter="sendMessage"
                                placeholder="Ask whatever you want..."
                                class="flex-1 px-4 py-2 text-sm text-gray-900 dark:text-white bg-transparent focus:outline-none">

                            <button @click="sendMessage" :disabled="!userInput.trim()"
                                :class="userInput.trim() ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-300 dark:bg-gray-700'"
                        ...
                        <template x-for="faq in filteredFaqs" :key="'dropdown-'+faq.id">
                            <div
                                ...
                                <button @click="openFaq = (openFaq === faq.id ? null : faq.id)"
                                    class="w-full flex items-center justify-between p-4 text-left bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <span class="text-sm font-medium text-gray-900 dark:text-white"
                                        x-text="faq.question"></span>
                                    <svg class="w-5 h-5 text-gray-500 transition-transform duration-200"
                                        :class="{'rotate-180': openFaq === faq.id}" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 9l-7 7-7-7" />
                                    </svg>
                                ...
                                <div x-show="openFaq === faq.id" x-collapse>
                                    <div
                                        class="p-4 pt-0 text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-900">
                ...
                {{-- Contact Section --}}
                <div
                    class="bg-none max-w-3xl p-4 border-t border-gray-100 dark:border-gray-700">
                    <div class="text-sm font-semibold text-gray-900 dark:text-white mb-1">Still have questions?</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400">Contact us: <span
                            class="font-bold text-indigo-600">+2355555888</span></div>
                </div>
            </div>
        </div>
    </div>
