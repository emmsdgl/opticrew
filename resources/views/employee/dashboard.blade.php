    ...
    <section role="status" class="hidden lg:flex flex-col lg:flex-row gap-6 p-4 md:p-6 min-h-[calc(100vh-4rem)]"
        ...
        <div x-show="showAttendanceDrawer" x-cloak class="fixed inset-0 z-50 overflow-hidden" style="display: none;">
            ...
            <div class="fixed inset-y-0 right-0 flex max-w-full">
                <div x-show="showAttendanceDrawer" x-transition:enter="transform transition ease-in-out duration-300"
                        ...
                        <div class="flex-1 overflow-y-auto p-6">
                            ...
                            <div class="mb-5">
                                <div class="py-3">
                                    <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">Attendance
                                        Details</h4>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">View your attendance time
                                        details for today</p>
                                </div>
                            ...
                            <div class="rounded-lg p-4 my-6 border-t border-gray-200 dark:border-gray-700">
                                <p class="text-sm text-center"
                                    ...
                                    @elseif($isClockedIn)
                                        <span><i class="fa-solid fa-spinner fa-spin mr-2"></i>You are currently <span
                                                class="font-semibold">clocked in</span></span>
                                    @else
        ...
        <div x-show="showRequestModal" x-cloak class="fixed inset-0 z-50 overflow-hidden" style="display: none;">
            ...
            <div x-show="showRequestModal" x-transition:enter="transition-opacity ease-out duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="transition-opacity ease-in duration-200" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0" @click="closeRequestModal()"
                class="absolute inset-0 bg-black/50 dark:bg-black/70"></div>
                        ...
                        <div class="flex-1 overflow-y-auto p-6 pt-3" x-show="selectedRequest">
                            ...
                            <template x-if="selectedRequest && selectedRequest.status === 'Approved'">
                                <div
                                    ...
                                    <span class="text-sm font-medium text-green-700 dark:text-green-400">This request
                            ...
                            <template x-if="selectedRequest && selectedRequest.status === 'Rejected'">
                                <div
                                    ...
                                    <span class="text-sm font-medium text-red-700 dark:text-red-400">This request has
                                        been declined</span>
                                </div>
                            </template>
                        </div>
