<x-layouts.general-dashboard :title="$title">
    ...
    @endphp
    <div x-data="managerProfileModal()" @open-profile-modal.window="openModal()">
        <template x-teleport="body">
            <div x-show="profileOpen" x-cloak
                ...
                <div x-show="profileOpen"
                     ...
                     @click="closeModal()" class="absolute inset-0 bg-black/50"></div>
                <div x-show="profileOpen"
                    ...
                    <x-material-ui.profile-card
                        :user="$profileUser"
                        coverUploadRoute="{{ route('manager.profile.upload-cover') }}"
                    />
                    <div class="px-6 pb-6">
    ...
    function managerProfileModal() {
        return {
            ...
            setPhonePrefix(prefix, max) { this.phoneStore[this.phonePrefix] = this.phoneLocal; this.phonePrefix = prefix; this.phoneMaxLen = max; this.phoneError = ''; this.phoneLocal = this.phoneStore[prefix] || ''; this.syncPhone(); },
            ...
            startEditing() { this.original = { ...this.form }; this.initPhone(); this.editing = true; window.dispatchEvent(new CustomEvent('profile-edit-toggled', { detail: { editing: true } })); },
            cancelEditing() {
                ...
                this.phoneError = ''; this.phoneDropOpen = false; this.editing = false;
            ...
            async saveProfile() {
                ...
                try {
                    ...
                    const res = await fetch('{{ route("manager.profile.update") }}', {
                        method: 'POST',
                    ...
                    if (res.ok && data.success) {
                        if (data.password_changed) { await window.showSuccessDialog('Password Changed', 'Your password has been changed. Please log in again.', 'OK'); window.location.href = '{{ route("login") }}'; return; }
